...
term.clear()
write("Downloading Phoenix...\n\nTip: The username and password are both ")
...
parallel.waitForAll(function()
    local handle = assert(http.get(url .. "kernel.lua", nil, true))
    kernel = assert(load(handle.readAll(), "=kernel", nil, _G))
    handle.close()
end, function()
    local handle = assert(http.get(url .. "rootfs.img", nil, true))
    _G._ROOTFS = textutils.unserialize(handle.readAll())
    handle.close()
end, function() sleep(3) end) -- minimum time so people can see the login info
...
function _G.term.native()
    ...
    os.shutdown = function()
        ...
        return kernel "initrd=_G._ROOTFS"
    end
end
if debug then
